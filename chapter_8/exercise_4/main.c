...
typedef enum { BEGINNING, CURRENT, END } Origin;
...
int myfflush(MYFILE *);
int myfclose(MYFILE *);
int myfseek(MYFILE *, long, Origin);
    ...
    if (argc == 1)
        ...
        while (--argc > 0)
            if ((fp = myfopen(*++argv, "r")) == NULL) {
                ...
                if (myfseek(fp, 64, BEGINNING) == -1) {
                    fprintf(stderr, "cat: failed to fseek %s\n", *argv);
                    exit(1);
                }
...
int myfseek(MYFILE *fp, long offset, Origin origin) {
    int result, n;
    result = 0;
